...
BATCH_SIZE = 50  # Number of states to include in experience replay
MAX_MEMORY = 50000  # Maximum number of experienced states to store in memory
...
class DQN:
    ...
    def setup_neural_net(self):
        ...
        visible1 = keras.layers.Input(shape=np.shape(scalars))
        fc1 = keras.layers.Dense(50, activation='relu')(visible1)

        previous_bandwidths = self.states[:, 3:4, :]
        visible2 = keras.layers.Input(shape=np.shape(previous_bandwidths))
        cnn1 = keras.layers.Conv1D(32, kernel_size=4, activation='relu')(visible2)
        flat1 = tf.layers.flatten(cnn1)

        # Merge the two parts into a single input
        full_net = keras.layers.merge.concatenate([fc1, flat1])

        fc2 = keras.layers.Dense(50, activation='relu')(full_net)

        output = keras.layers.Dense(self.num_actions, activation='relu')(fc2)
        ...
        loss = tf.losses.mean_squared_error(self.action_values, output)
